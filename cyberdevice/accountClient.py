...
create a temp user, will be deleted after specified expire_in duration
    ...
    """
    # note that the server is checking and deleting the expired user in fixed time interval (ex. 5 min)
    # so the expire_in may not accurate
    # the expire_in is at least 300 sec and at most 90 days, the server will also check this
    ...
    if r.status_code == 200:
        res = json.loads(r.content)
        print("temp User '{}' id='{}' has been created successfully !".format(username, res['user_id']))
        return res['user_id']
...
create a user
    ...
    if r.status_code == 200:
        res = json.loads(r.content)
        print("User '{}' id='{}' has been created successfully !".format(username, res['user_id']))
        return res['user_id']
    elif r.status_code == 401:
        raise InvalidTokenException(r.text)
    elif r.status_code == 403:
...
reset the password of a target user
    ...
    r = requests.post(url, data=json.dumps(payload), headers=headers)
    if r.status_code == 200:
        print("the password of temp User id='{}' has been reset successfully !".format(user_id))
    elif r.status_code == 401:
        raise InvalidTokenException(r.text)
    elif r.status_code == 403:
        raise PermissionException(r.text)
...
def oauth2_authorize(api_token: str, authorize_uri, client_id: str, redirect_uri):
    url = authorize_uri + "?response_type=code&client_id=" +\
          client_id + "&scope=openid&redirect_uri=" + quote(redirect_uri)
...
def oauth2_token_request(token_uri, client_id: str, client_secret: str, code: str, redirect_uri):
    url = token_uri
    credential = quote(client_id) + ":" + quote(client_secret)
    headers = {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8',
               "Authorization": b"Basic "+base64.b64encode(credential.encode("utf-8"))}
    data = 'grant_type=authorization_code&code='+code+'&redirect_uri='+quote(redirect_uri)
